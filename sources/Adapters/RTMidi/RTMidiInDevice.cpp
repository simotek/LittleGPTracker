...
void mycallback(double deltatime, std::vector<unsigned char> *message,
    ...
    // Protect against empty messages crashing std::vector::at()
    if (nBytes == 0)
        return;
...
RTMidiInDevice::RTMidiInDevice(RtMidiIn &in, int index, const char *name)
    : MidiInDevice(name), rtMidiIn_(in.getCurrentApi(), "LittleGPTracker"),
      index_(index) {};
...
bool RTMidiInDevice::initDriver() {
    ...
    rtMidiIn_.openPort(index_, "LittleGPTracker");
...
bool RTMidiInDevice::startDriver() { return true; };
